...
export function AdminLayout({ title, description, children }: AdminLayoutProps) {
  ...
  if (loading || (user && accessState === "checking")) {
    return <section className="rounded-xl bg-white p-4 text-sm text-zinc-500 shadow-sm">正在检查管理员权限...</section>;
  }
  ...
  if (!user) {
    return (
      <section className="rounded-xl bg-white p-4 shadow-sm">
        ...
        <p className="mt-3 text-sm text-zinc-600">{error || "当前未登录，请先登录后访问管理后台。"}</p>
  ...
  if (accessState !== "allowed") {
    return (
      <section className="rounded-xl bg-white p-4 shadow-sm">
        <h1 className="text-lg font-semibold">管理后台</h1>
        <p className="mt-3 text-sm text-zinc-600">当前账号没有管理员权限，无法访问用户管理后台。</p>
