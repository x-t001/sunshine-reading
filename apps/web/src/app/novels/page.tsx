...
export default async function NovelsPage({ searchParams }: NovelsPageProps) {
  ...
  let novelPage: Awaited<ReturnType<typeof getNovels>> | null = null;
  let loadError: unknown = null;

  try {
    [categories, novelPage] = await Promise.all([
      getCategories(),
  ...
  if (loadError || !categories || !novelPage) {
    return (
      <section className="rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-700">
        小说列表加载失败：{getApiErrorMessage(loadError)}
      </section>
    );
  }
