...
import { approveNovel, getReviewingNovels, rejectNovel } from "@/lib/api/reviewer";
...
export default function ReviewerReviewingNovelsPage() {
  return (
    <ReviewerLayout title="我的作品审核" description="继续处理已领取但尚未完成的作品审核任务。">
      <ReviewerReviewingNovelsContent />
    </ReviewerLayout>
  );
}

function ReviewerReviewingNovelsContent() {
  ...
  const loadNovels = useCallback(async () => {
    ...
    try {
      ...
      const result = await getReviewingNovels(params);
  ...
  async function handleApprove(id: number) {
    ...
    setNotice(null);
    try {
      const result = await approveNovel(id);
      setNotice(`作品“${result.title}”已通过，当前状态：${auditStatusLabels[result.audit_status]}`);
      await loadNovels();
    } catch (actionError) {
  ...
  return (
    <div className="space-y-4">
      ...
      {loading ? <p className="rounded-xl bg-white p-4 text-sm text-zinc-500 shadow-sm">正在加载我的作品审核...</p> : null}

      {!loading && items.length === 0 ? (
        <section className="rounded-xl border border-dashed border-zinc-300 bg-white p-6 text-center text-sm text-zinc-500">暂无审核中的作品。</section>
      ) : null}
            ...
            <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
              <div className="min-w-0">
                ...
                <p className="mt-2 text-xs text-zinc-500">
                  {formatWordCount(novel.word_count)} · 审核员：{novel.reviewer?.nickname || novel.reviewer?.username || "暂无"}
                </p>
              ...
              <div className="flex flex-wrap gap-2 text-sm">
                ...
                <button
                  ...
                  onClick={() => void handleApprove(novel.id)}
                >
                  通过
                </button>
                <button
